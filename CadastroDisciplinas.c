...
int cadastra_disciplina(int qtde_Disciplinas, Disciplina lista_disciplinas[], Lista_Prof Professor[]);
...
int incluir_aluno(Disciplina lista_disciplinas[], int qtde_Disciplinas, Lista_aluno Aluno[]);
int localizar_Alunos(int loc_matricula,  Lista_aluno Aluno[]);
void zerando_contador(Disciplina lista_disciplinas[]);
void imprime_disciplina_com_aluno(Disciplina lista_disciplinas[], int qtde_Disciplinas,  Lista_aluno Aluno[]);
...
int localizar_Professores(int loc_matricula,  Lista_Prof Professor[]);
void alterar_cadastro_disciplina(Disciplina lista_disciplinas[], int qtde_Disciplinas, Lista_Prof Professor[]);
...
int main() {
    ...
    Lista_aluno Aluno[TAM_ALUNOS];
    Lista_Prof Professor[TAM_PROFESSOR];
    int i;
    ...
    switch(op){
      ...
      case 1: {
        retorno = cadastra_disciplina(qtd_disciplinas, lista_disciplinas, Professor);
    ...
    case 4:{
        retorno=incluir_aluno(lista_disciplinas, qtd_disciplinas, Aluno);
        if (retorno == SUCESSO_CADASTRO)
            printf("\n\n##Cadastro realizado com sucesso##\n\n");
    ...
    case 7: {
        imprime_disciplina_com_aluno(lista_disciplinas, qtd_disciplinas,Aluno);
        break;
      }
      case 5:{
      ...
      case 2:{
          alterar_cadastro_disciplina(lista_disciplinas, qtd_disciplinas, Professor);
          break;
        }
      default:{
...
int cadastra_disciplina(int qtde_Disciplinas, Disciplina lista_disciplinas[], Lista_Prof Professor[]){
    ...
    do{
        ...
        getchar();
        verificador = localizar_Professores(matricula_professor, Professor);
        if(verificador== NAO_LOCALIZADO)
            printf("##PROFESSOR NÃO LOCALIZADO. DIGITE NOVAMENTE##\n");
        else
            strcpy(lista_disciplinas[qtde_Disciplinas].professor_Disciplina, Professor[verificador].nome);
...
void imprime_disciplina_com_aluno(Disciplina lista_disciplinas[], int qtde_Disciplinas,  Lista_aluno Aluno[]){
    ...
    for (iCont = 0; iCont < qtde_Disciplinas; iCont++){
        ...
        for(jCont = 0; jCont < lista_disciplinas[iCont].contador; jCont++){
				pos_aluno= localizar_Alunos(lista_disciplinas[iCont].alunos_matriculados[jCont], Aluno);
				printf("\t\tMatricula: %d\tNome: %s\n", Aluno[pos_aluno].matricula, Aluno[pos_aluno].nome);
			}
        printf("--------------------------------------------------\n");
	}
...
int incluir_aluno(Disciplina lista_disciplinas[], int qtde_Disciplinas, Lista_aluno Aluno[]){
    ...
    do{
        ...
        posicao_aluno= localizar_Alunos(matricula_aluno, Aluno);
    ...
    lista_disciplinas[posicao_disciplina].alunos_matriculados[lista_disciplinas[posicao_disciplina].contador] = Aluno[posicao_aluno].matricula; 
...
int localizar_Professores(int loc_matricula,  Lista_Prof Professor[]){
    ...
    for (X = 0; X < TAM_PROFESSOR ; X++) {
        if (Professor[X].matricula == loc_matricula) {
            return X;
        }
    }
    return NAO_LOCALIZADO;
    ...
    switch(opcao)
    {
        ...
        case 4:
        {
            ...
            do{
                ...
                getchar();
                verificador = localizar_Professores(alt_professor, Professor);
                if(verificador== NAO_LOCALIZADO)
                    printf("PROFESSOR NÃO LOCALIZADO. DIGITE NOVAMENTE!!\n");
                else{
                    strcpy(lista_disciplinas[posicao_disciplina].professor_Disciplina, Professor[verificador].nome);
                    printf("\n###ALTERAÇÃO REALIZADA COM SUCESSO###\n");
                }
            }while(verificador == NAO_LOCALIZADO);
